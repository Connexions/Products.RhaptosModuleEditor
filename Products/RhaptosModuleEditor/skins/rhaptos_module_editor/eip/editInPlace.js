...
/**
 * Used to clear the errors/warnings generated by the importer
 * Code that calls this is in RhaptosModuleEditor/www/editInPlace.xsl
 */
function clearAllMessages() {
  xmlText = document.getElementById('textarea');
  xmlText.value = xmlText.value.replace(/<\?cnx.[^?>]*\?>/g,'');

  // By default, submitting to the full-source form will take you back to full source edit
  // EIP on the other hand will only let you submit snippet changes
  // since we're changing the entire document, we need to submit as if
  // the user was using full source but want them to be redirected to eip
  // hence the need to add a "edit_source" input field

  form = document.getElementById('edit_form');
  editSource = document.createElement('input');
  editSource.name = 'edit_source:int';
  editSource.value = '0';
  form.appendChild(editSource);
